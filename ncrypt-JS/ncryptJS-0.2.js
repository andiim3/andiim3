 ...
 * 
 *  Version 0.2 - Base64, TextEncode, and LZW compression. (No MD5 used)
 * 
...
class ncrypt {
    encrypt (key, txt){
        ...
        base64txt = b64key+btoa(base64txt)+b64key;        
        base64txt = btoa(base64txt);
        return hash+this.lzw_encode(base64txt);
    }

    decrypt(key, txt){        
        var b64key = btoa(key);
        var hash=this.hashit(key);
        var encryptedhash = txt.substring(0,32);
        var decrypted = txt;
        if(hash==encryptedhash){
            decrypted=decrypted.replace(hash, "");
            decrypted = atob(this.lzw_decode(decrypted))        
            decrypted=decrypted.replace(b64key, "");        
            decrypted=decrypted.replace(b64key, "");
            decrypted=atob(decrypted);
            decrypted=this.textdecode(decrypted);
            return decrypted;
        }else{
            return "Wrong Key!";
        }
